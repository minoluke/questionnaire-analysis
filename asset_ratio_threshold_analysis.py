"""
月収に対する自由資産率0.25を境界とした利用意向の違い分析
"""
...
def analyze_asset_ratio_threshold(data_file=None, output_dir=None):
    """月収に対する自由資産率0.25を境界とした利用意向の違い分析"""
    ...
    if output_dir is None:
        output_dir = "output/asset_ratio_threshold_analysis"
    ...
    print("=== 月収に対する自由資産率0.25境界分析 ===")
    print(f"データ: {data_file}")
    print(f"サンプル数: {len(data)}件")
    print(f"全体利用意向率: {overall_intent_rate:.1%}")
    
    # 条件設定
    below_025 = data['月収に対する自由資産率'] < 0.25
    above_ge_025 = data['月収に対する自由資産率'] >= 0.25
    
    # 各グループの該当者数
    below_count = below_025.sum()
    above_count = above_ge_025.sum()
    
    print(f"\n=== グループ分析 ===")
    print(f"月収に対する自由資産率 < 0.25: {below_count}件 ({below_count/len(data)*100:.1f}%)")
    print(f"月収に対する自由資産率 ≥ 0.25: {above_count}件 ({above_count/len(data)*100:.1f}%)")
    
    # 利用意向率の計算
    below_intent_rate = data[below_025][target_column].mean() if below_count > 0 else 0
    above_intent_rate = data[above_ge_025][target_column].mean() if above_count > 0 else 0
    
    print(f"\n=== 利用意向率 ===")
    print(f"月収に対する自由資産率 < 0.25: {below_intent_rate:.1%}")
    print(f"月収に対する自由資産率 ≥ 0.25: {above_intent_rate:.1%}")
    ...
    print(f"\n=== Lift ===")
    print(f"月収に対する自由資産率 < 0.25: {below_lift:.2f}")
    print(f"月収に対する自由資産率 ≥ 0.25: {above_lift:.2f}")
    
    # 信頼区間
    if below_count > 0:
        below_successes = data[below_025][target_column].sum()
        below_ci_low, below_ci_high = proportion_confint(below_successes, below_count, method='wilson')
    
    if above_count > 0:
        above_successes = data[above_ge_025][target_column].sum()
        above_ci_low, above_ci_high = proportion_confint(above_successes, above_count, method='wilson')
    
    print(f"\n=== 信頼区間（95%） ===")
    print(f"月収に対する自由資産率 < 0.25: [{below_ci_low:.1%}, {below_ci_high:.1%}]")
    print(f"月収に対する自由資産率 ≥ 0.25: [{above_ci_low:.1%}, {above_ci_high:.1%}]")
    
    # カイ二乗検定
    if below_count > 0 and above_count > 0:
        below_intent = data[below_025][target_column].sum()
        below_no_intent = below_count - below_intent
        
        above_intent = data[above_ge_025][target_column].sum()
        ...
        print(f"月収に対する自由資産率<0.25    {below_intent:3d}        {below_no_intent:3d}")
        print(f"月収に対する自由資産率≥0.25    {above_intent:3d}        {above_no_intent:3d}")
    ...
    # t検定も実施
    if below_count > 0 and above_count > 0:
        below_data = data[below_025][target_column]
        above_data = data[above_ge_025][target_column]
    ...
    for threshold in thresholds:
        below_threshold = data['月収に対する自由資産率'] < threshold
        above_threshold = data['月収に対する自由資産率'] >= threshold
        ...
        print(f"閾値{threshold}: < {below_th_rate:.1%} ({below_th_count}件) vs ≥ {above_th_rate:.1%} ({above_th_count}件), 差: {diff:.1%}")
    
    # プロット作成
    fig, axes = plt.subplots(2, 3, figsize=(18, 12))
    fig.suptitle('月収に対する自由資産率0.25境界分析結果', fontsize=16, fontweight='bold')
    ...
    ax1.set_title('月収に対する自由資産率0.25境界での利用意向率')
    ...
    # 3. 自由資産率分布（利用意向別）
    ax3 = axes[0, 2]
    intent_0_data = data[data[target_column] == 0]['月収に対する自由資産率']
    intent_1_data = data[data[target_column] == 1]['月収に対する自由資産率']
    
    # データの範囲を確認して適切なbinsを設定
    max_ratio = 0.8  # 固定値で設定
    bins = np.linspace(0, max_ratio, 15)
    ...
    ax3.axvline(x=0.25, color='green', linestyle='--', linewidth=2, label='境界値0.25')
    ax3.set_xlabel('月収に対する自由資産率')
    ax3.set_ylabel('密度')
    ax3.set_title('自由資産率分布（利用意向別）')
    ax3.legend()
    ...
    ax4.axvline(x=0.25, color='green', linestyle='--', alpha=0.7, label='境界値0.25')
    ax4.set_xlabel('自由資産率閾値')
    ...
    # 6. 箱ひげ図（利用意向別の自由資産率分布）
    ax6 = axes[1, 2]
    box_data = [
        data[data[target_column] == 0]['月収に対する自由資産率'].values,
        data[data[target_column] == 1]['月収に対する自由資産率'].values
    ]
    ...
    # 境界線を追加
    ax6.axhline(y=0.25, color='green', linestyle='--', alpha=0.7, label='境界値0.25')
    
    ax6.set_ylabel('月収に対する自由資産率')
    ax6.set_title('利用意向別自由資産率の分布')
    ax6.legend()
    ...
    # 保存
    output_file = output_path / "asset_ratio_threshold_analysis.png"
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    plt.show()
    print(f"\n可視化結果保存: {output_file}")
    
    # レポート保存
    report_file = output_path / "asset_ratio_threshold_report.md"
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("# 月収に対する自由資産率0.25境界分析レポート\n\n")
        
        f.write("## 基本統計\n")
        f.write(f"- **自由資産率 < 0.25**: {below_count}件 ({below_count/len(data)*100:.1f}%), 利用意向率 {below_intent_rate:.1%}\n")
        f.write(f"- **自由資産率 ≥ 0.25**: {above_count}件 ({above_count/len(data)*100:.1f}%), 利用意向率 {above_intent_rate:.1%}\n")
        ...
        f.write("## 信頼区間\n")
        f.write(f"- **自由資産率 < 0.25**: [{below_ci_low:.1%}, {below_ci_high:.1%}]\n")
        f.write(f"- **自由資産率 ≥ 0.25**: [{above_ci_low:.1%}, {above_ci_high:.1%}]\n\n")
        
        f.write("## 閾値別分析\n")
        for result in threshold_results:
            f.write(f"- **閾値{result['threshold']}**: 差 {result['difference']:.1%} "
...
if __name__ == "__main__":
    ...
    analyze_asset_ratio_threshold(data_file, output_dir)