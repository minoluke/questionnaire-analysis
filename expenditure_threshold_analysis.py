...
def analyze_expenditure_threshold(data_file=None, output_dir=None):
    """支出変動率0.5を境界とした利用意向の違い分析"""
    ...
    if output_dir is None:
        output_dir = "output/expenditure_threshold_analysis"
    ...
    # 条件設定
    below_05 = data['支出変動率'] < 0.5
    above_ge_05 = data['支出変動率'] >= 0.5
    
    # 各グループの該当者数
    below_count = below_05.sum()
    above_count = above_ge_05.sum()
    
    print(f"\n=== グループ分析 ===")
    print(f"支出変動率 < 0.5: {below_count}件 ({below_count/len(data)*100:.1f}%)")
    print(f"支出変動率 ≥ 0.5: {above_count}件 ({above_count/len(data)*100:.1f}%)")
    
    # 利用意向率の計算
    below_intent_rate = data[below_05][target_column].mean() if below_count > 0 else 0
    above_intent_rate = data[above_ge_05][target_column].mean() if above_count > 0 else 0
    
    print(f"\n=== 利用意向率 ===")
    print(f"支出変動率 < 0.5: {below_intent_rate:.1%}")
    print(f"支出変動率 ≥ 0.5: {above_intent_rate:.1%}")
    ...
    print(f"\n=== Lift ===")
    print(f"支出変動率 < 0.5: {below_lift:.2f}")
    print(f"支出変動率 ≥ 0.5: {above_lift:.2f}")
    
    # 信頼区間
    if below_count > 0:
        below_successes = data[below_05][target_column].sum()
        below_ci_low, below_ci_high = proportion_confint(below_successes, below_count, method='wilson')
    
    if above_count > 0:
        above_successes = data[above_ge_05][target_column].sum()
        above_ci_low, above_ci_high = proportion_confint(above_successes, above_count, method='wilson')
    
    print(f"\n=== 信頼区間（95%） ===")
    print(f"支出変動率 < 0.5: [{below_ci_low:.1%}, {below_ci_high:.1%}]")
    print(f"支出変動率 ≥ 0.5: [{above_ci_low:.1%}, {above_ci_high:.1%}]")
    
    # カイ二乗検定
    if below_count > 0 and above_count > 0:
        below_intent = data[below_05][target_column].sum()
        below_no_intent = below_count - below_intent
        
        above_intent = data[above_ge_05][target_column].sum()
        ...
        print(f"支出変動率<0.5        {below_intent:3d}        {below_no_intent:3d}")
        print(f"支出変動率≥0.5        {above_intent:3d}        {above_no_intent:3d}")
    ...
    # t検定も実施
    if below_count > 0 and above_count > 0:
        below_data = data[below_05][target_column]
        above_data = data[above_ge_05][target_column]
    ...
    for threshold in thresholds:
        below_threshold = data['支出変動率'] < threshold
        above_threshold = data['支出変動率'] >= threshold
    ...
    fig.suptitle('支出変動率0.5境界分析結果', fontsize=16, fontweight='bold')
    ...
    ax1.set_title('支出変動率0.5境界での利用意向率')
    ...
    # 3. 支出変動率分布（利用意向別）
    ax3 = axes[0, 2]
    intent_0_data = data[data[target_column] == 0]['支出変動率']
    intent_1_data = data[data[target_column] == 1]['支出変動率']
    
    bins = np.linspace(0, data['支出変動率'].max(), 20)
    ...
    ax3.axvline(x=0.5, color='green', linestyle='--', linewidth=2, label='境界値0.5')
    ax3.set_xlabel('支出変動率')
    ax3.set_ylabel('密度')
    ax3.set_title('支出変動率分布（利用意向別）')
    ax3.legend()
    ...
    ax4.axvline(x=0.5, color='green', linestyle='--', alpha=0.7, label='境界値0.5')
    ax4.set_xlabel('支出変動率閾値')
    ...
    # 6. 箱ひげ図
    ax6 = axes[1, 2]
    box_data = [data[below_05]['支出変動率'].values, data[above_ge_05]['支出変動率'].values]
    bp = ax6.boxplot(box_data, labels=['利用意向なし\n(< 0.5多数)', '利用意向あり\n(≥ 0.5多数)'], patch_artist=True)
    bp['boxes'][0].set_facecolor('lightblue')
    bp['boxes'][1].set_facecolor('lightcoral')
    ax6.set_ylabel('支出変動率')
    ax6.set_title('支出変動率の箱ひげ図')
    ax6.grid(True, alpha=0.3)
    
    plt.tight_layout()
    
    # 保存
    output_file = output_path / "expenditure_threshold_analysis.png"
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    plt.show()
    print(f"\n可視化結果保存: {output_file}")
    
    # レポート保存
    report_file = output_path / "expenditure_threshold_report.md"
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("# 支出変動率0.5境界分析レポート\n\n")
        
        f.write("## 基本統計\n")
        f.write(f"- **支出変動率 < 0.5**: {below_count}件 ({below_count/len(data)*100:.1f}%), 利用意向率 {below_intent_rate:.1%}\n")
        f.write(f"- **支出変動率 ≥ 0.5**: {above_count}件 ({above_count/len(data)*100:.1f}%), 利用意向率 {above_intent_rate:.1%}\n")
        ...
        f.write("## 信頼区間\n")
        f.write(f"- **支出変動率 < 0.5**: [{below_ci_low:.1%}, {below_ci_high:.1%}]\n")
        f.write(f"- **支出変動率 ≥ 0.5**: [{above_ci_low:.1%}, {above_ci_high:.1%}]\n\n")
...
if __name__ == "__main__":
    ...
    analyze_expenditure_threshold(data_file, output_dir)