...
def analyze_conditions_40_60_18_50(data_file=None, output_dir=None):
    """最高月収40-60万円 & 収入変動額18-50万円の詳細分析"""
    
    # デフォルト設定
    if data_file is None:
        data_file = "data/processed/preprocessed_100data.csv"
    if output_dir is None:
        output_dir = "output/condition_40_60_18_50_analysis"
    ...
    fig.suptitle('条件分析結果\n最高月収40-60万円 & 収入変動額18-50万円', fontsize=16, fontweight='bold')
    ...
    # 条件範囲をハイライト
    rect = plt.Rectangle((40, 18), 20, 32, fill=False, edgecolor='green', linewidth=3, linestyle='--')
    ax1.add_patch(rect)
    ...
    # 保存
    output_file = output_path / "condition_40_60_18_50_analysis.png"
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    plt.show()
    print(f"\n可視化結果保存: {output_file}")
    
    # レポート保存
    report_file = output_path / "condition_40_60_18_50_report.md"
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("# 条件分析レポート: 最高月収40-60万円 & 収入変動額18-50万円\n\n")
        
        f.write("## 条件設定\n")
        f.write("- **最高月収**: 40 ≤ x ≤ 60万円\n")
...
if __name__ == "__main__":
    ...
    analyze_conditions_40_60_18_50(data_file, output_dir)