...
def analyze_max_income_threshold(data_file=None, output_dir=None):
    """最高月収40万円を境界とした利用意向の違い分析"""
    ...
    if output_dir is None:
        output_dir = "output/max_income_threshold_analysis"
    ...
    # 条件設定
    below_40 = data['最高月収'] < 40
    above_ge_40 = data['最高月収'] >= 40
    
    # 各グループの該当者数
    below_count = below_40.sum()
    above_count = above_ge_40.sum()
    
    print(f"\n=== グループ分析 ===")
    print(f"最高月収 < 40万円: {below_count}件 ({below_count/len(data)*100:.1f}%)")
    print(f"最高月収 ≥ 40万円: {above_count}件 ({above_count/len(data)*100:.1f}%)")
    
    # 利用意向率の計算
    below_intent_rate = data[below_40][target_column].mean() if below_count > 0 else 0
    above_intent_rate = data[above_ge_40][target_column].mean() if above_count > 0 else 0
    
    print(f"\n=== 利用意向率 ===")
    print(f"最高月収 < 40万円: {below_intent_rate:.1%}")
    print(f"最高月収 ≥ 40万円: {above_intent_rate:.1%}")
    ...
    print(f"\n=== Lift ===")
    print(f"最高月収 < 40万円: {below_lift:.2f}")
    print(f"最高月収 ≥ 40万円: {above_lift:.2f}")
    
    # 信頼区間
    if below_count > 0:
        below_successes = data[below_40][target_column].sum()
        below_ci_low, below_ci_high = proportion_confint(below_successes, below_count, method='wilson')
    
    if above_count > 0:
        above_successes = data[above_ge_40][target_column].sum()
        above_ci_low, above_ci_high = proportion_confint(above_successes, above_count, method='wilson')
    
    print(f"\n=== 信頼区間（95%） ===")
    print(f"最高月収 < 40万円: [{below_ci_low:.1%}, {below_ci_high:.1%}]")
    print(f"最高月収 ≥ 40万円: [{above_ci_low:.1%}, {above_ci_high:.1%}]")
    
    # カイ二乗検定
    if below_count > 0 and above_count > 0:
        below_intent = data[below_40][target_column].sum()
        below_no_intent = below_count - below_intent
        
        above_intent = data[above_ge_40][target_column].sum()
        ...
        print(f"最高月収<40万円       {below_intent:3d}        {below_no_intent:3d}")
        print(f"最高月収≥40万円       {above_intent:3d}        {above_no_intent:3d}")
    ...
    # t検定も実施
    if below_count > 0 and above_count > 0:
        below_data = data[below_40][target_column]
        above_data = data[above_ge_40][target_column]
    ...
    for threshold in thresholds:
        below_threshold = data['最高月収'] < threshold
        above_threshold = data['最高月収'] >= threshold
    ...
    fig.suptitle('最高月収40万円境界分析結果', fontsize=16, fontweight='bold')
    ...
    ax1.set_title('最高月収40万円境界での利用意向率')
    ...
    # 3. 最高月収分布（利用意向別）
    ax3 = axes[0, 2]
    intent_0_data = data[data[target_column] == 0]['最高月収']
    intent_1_data = data[data[target_column] == 1]['最高月収']
    
    # データの範囲を確認して適切なbinsを設定
    max_income = min(data['最高月収'].quantile(0.95), 150)  # 95%点または150の小さい方
    bins = np.linspace(0, max_income, 15)
    ...
    ax3.axvline(x=40, color='green', linestyle='--', linewidth=2, label='境界値40万円')
    ax3.set_xlabel('最高月収（万円）')
    ax3.set_ylabel('密度')
    ax3.set_title('最高月収分布（利用意向別）')
    ax3.legend()
    ...
    ax4.axvline(x=40, color='green', linestyle='--', alpha=0.7, label='境界値40万円')
    ax4.set_xlabel('最高月収閾値（万円）')
    ...
    ax6 = axes[1, 2]
    box_data = [
        data[data[target_column] == 0]['最高月収'].values,
        data[data[target_column] == 1]['最高月収'].values
    ]
    ...
    # 境界線を追加
    ax6.axhline(y=40, color='green', linestyle='--', alpha=0.7, label='境界値40万円')
    
    ax6.set_ylabel('最高月収（万円）')
    ax6.set_title('利用意向別最高月収の分布')
    ax6.legend()
    ...
    # 保存
    output_file = output_path / "max_income_threshold_analysis.png"
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    plt.show()
    print(f"\n可視化結果保存: {output_file}")
    
    # レポート保存
    report_file = output_path / "max_income_threshold_report.md"
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("# 最高月収40万円境界分析レポート\n\n")
        
        f.write("## 基本統計\n")
        f.write(f"- **最高月収 < 40万円**: {below_count}件 ({below_count/len(data)*100:.1f}%), 利用意向率 {below_intent_rate:.1%}\n")
        f.write(f"- **最高月収 ≥ 40万円**: {above_count}件 ({above_count/len(data)*100:.1f}%), 利用意向率 {above_intent_rate:.1%}\n")
        ...
        f.write("## 信頼区間\n")
        f.write(f"- **最高月収 < 40万円**: [{below_ci_low:.1%}, {below_ci_high:.1%}]\n")
        f.write(f"- **最高月収 ≥ 40万円**: [{above_ci_low:.1%}, {above_ci_high:.1%}]\n\n")
...
if __name__ == "__main__":
    ...
    analyze_max_income_threshold(data_file, output_dir)