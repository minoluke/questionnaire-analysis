"""
CART分析実行スクリプト
scikit-learnのCARTアルゴリズムを使用して実用的なルールを抽出
"""
...
def main():
    """CART分析メイン実行関数"""
    print("=== CART分析（決定木）を開始します ===")
    
    # 前処理済みデータの読み込み
    input_file = "data/processed/preprocessed_data.csv"
    ranking_file = "output/tables/univariate_ranking_table.csv"
    table_file = "output/tables/cart_rules_table.csv"
    report_file = "output/reports/cart_rules_report.md"
    tree_text_file = "output/reports/cart_tree_structure.txt"
    tree_viz_file = "output/reports/cart_tree_visualization.png"
    ...
    print(f"\n=== 使用する特徴量 ===")
    print(f"上位6個の数値型特徴量: {top_numerical_features}")
    ...
    # 複数の設定で分析を試行
    cart_configs = [
        {'max_depth': 2, 'min_samples_split': 3, 'min_samples_leaf': 2, 'name': '浅い決定木'},
        {'max_depth': 3, 'min_samples_split': 3, 'min_samples_leaf': 2, 'name': '中程度の決定木'},
        {'max_depth': 4, 'min_samples_split': 2, 'min_samples_leaf': 1, 'name': '深い決定木'}
    ]
    ...
    for config in cart_configs:
        ...
        if analyzer.fit_tree(data, top_numerical_features, target_column):
            ...
            if rules:
                ...
                if len(practical_rules) > best_rules_count:
                    best_rules_count = len(practical_rules)
